...
class PaymentScheduleSortInput:
    ...
    by: Optional[PaymentScheduleSortBy] = dataclasses.field(metadata={"serde_skip_if": lambda value: value is None})
    """정렬 기준 필드
    
    어떤 필드를 기준으로 정렬할 지 결정합니다. 비워서 보낼 경우, TIME_TO_PAY가 기본값으로 설정됩니다.
    """
    order: Optional[SortOrder] = dataclasses.field(metadata={"serde_skip_if": lambda value: value is None})
    """정렬 순서
