...
class CancelPaymentBody:
    ...
    """
    amount: Optional[int] = dataclasses.field()
    """취소 총 금액
    ...
    """취소 사유"""
    requester: Optional[CancelRequester] = dataclasses.field()
    """취소 요청자
    
    고객에 의한 취소일 경우 Customer, 관리자에 의한 취소일 경우 Admin으로 입력합니다.
    """
    current_cancellable_amount: Optional[int] = dataclasses.field(metadata={"serde_rename": "currentCancellableAmount"})
    """결제 건의 취소 가능 잔액
    
    본 취소 요청 이전의 취소 가능 잔액으로써, 값을 입력하면 잔액이 일치하는 경우에만 취소가 진행됩니다. 값을 입력하지 않으면 별도의 검증 처리를 수행하지 않습니다.
    """
    refund_account: Optional[CancelPaymentBodyRefundAccount] = dataclasses.field(metadata={"serde_rename": "refundAccount"})
    """환불 계좌
