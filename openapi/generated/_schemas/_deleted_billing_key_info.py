...
class DeletedBillingKeyInfo:
    """빌링키 삭제 완료 상태 건"""
    status: Literal["DELETED"] = dataclasses.field()
    """빌링키 상태"""
    ...
    """상점 아이디"""
    methods: Optional[list[BillingKeyPaymentMethod]] = dataclasses.field()
    """빌링키 결제수단 상세 정보
    ...
    """사용자 지정 데이터"""
    issue_id: Optional[str] = dataclasses.field(metadata={"serde_rename": "issueId"})
    """고객사가 채번하는 빌링키 발급 건 고유 아이디"""
    issue_name: Optional[str] = dataclasses.field(metadata={"serde_rename": "issueName"})
    """빌링키 발급 건 이름"""
    ...
    """채널 그룹"""
    pg_billing_key_issue_responses: Optional[list[PgBillingKeyIssueResponse]] = dataclasses.field(metadata={"serde_rename": "pgBillingKeyIssueResponses"})
    """채널 별 빌링키 발급 응답
