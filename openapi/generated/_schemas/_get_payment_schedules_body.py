...
class GetPaymentSchedulesBody:
    ...
    """
    page: Optional[PageInput] = dataclasses.field(metadata={"serde_skip_if": lambda value: value is None})
    """요청할 페이지 정보
    
    미 입력 시 number: 0, size: 10 으로 기본값이 적용됩니다.
    """
    sort: Optional[PaymentScheduleSortInput] = dataclasses.field(metadata={"serde_skip_if": lambda value: value is None})
    """정렬 조건
    
    미 입력 시 sortBy: TIME_TO_PAY, sortOrder: DESC 으로 기본값이 적용됩니다.
    """
    filter: Optional[PaymentScheduleFilterInput] = dataclasses.field(metadata={"serde_skip_if": lambda value: value is None})
    """조회할 결제 예약 건의 조건 필터"""
